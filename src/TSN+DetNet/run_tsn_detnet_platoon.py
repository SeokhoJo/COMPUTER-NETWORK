...
print("### TSN + DETNET PLATOON CODE RUNNING - OMNeT++/INET RESULT DRIVEN ###")
...
RESULT_DIR = os.path.join(RESULT_ROOT, "tsn_detnet")
LOG_DIR = os.path.join(RESULT_DIR, "tsn_detnet_logs")
SUMMARY_FILE = os.path.join(RESULT_DIR, "tsn_detnet_summary.csv")
...
OMNET_RUNTIME_RESULT_DIR = os.path.join(
    OMNET_TSN_DIR, "results", "sumo_tsn_detnet_runtime"
)
...
# DetNet path model
# 기본 DetNet normal은 path A/B 모두 정상
PATH_A_FAILED_RECEIVERS = set()
...
def run_omnet_tsn():
    ...
    DetNet normal은 TSN 저지연 결과에 경로 복제 개념을 더하는 방식으로 사용한다.
    """
...
def decide_detnet_reaction(veh_id: str, detnet_delay_sec: float):
    ...
    - 둘 중 하나라도 성공하면 detnet_delay_sec로 반응
    - 둘 다 실패하면 fallback 지연
    """
...
def run_single_spacing(spacing_m: float):
    print("\n==============================")
    print(f"TSN + DETNET SCENARIO spacing = {spacing_m} m")
    print("==============================")
    ...
    while step < MAX_STEPS:
        ...
        if visible and not event_triggered and all_visible_reference_distance is not None:
            ...
            if (lead_dist - all_visible_reference_distance) >= EVENT_EXTRA_TRAVEL_AFTER_ALL_VISIBLE:
                ...
                print("[OMNeT++] Running INET TSN simulation for DetNet base delay...")
                ...
                print(f"[DETNET] path A failed receivers = {sorted(PATH_A_FAILED_RECEIVERS)}")
                print(f"[DETNET] path B failed receivers = {sorted(PATH_B_FAILED_RECEIVERS)}")
                ...
                for i in range(1, VEHICLE_COUNT):
                    ...
                    print(
                        f"[DETNET OMNET DECISION] {veh} -> path={path_used}, {detail}, "
        ...
        if stop_confirmed_step is not None and step >= stop_confirmed_step + POST_STOP_HOLD_STEPS:
            print(f"[END] TSN+DetNet scenario {spacing_m}m finished")
            break
...
def main():
    ...
    print("\nDONE: tsn_detnet_summary.csv and tsn_detnet_logs/*.csv generated")
