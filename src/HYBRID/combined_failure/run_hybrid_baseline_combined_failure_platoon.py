...
print("### HYBRID BASELINE COMBINED FAILURE CODE RUNNING ###")
...
RESULT_DIR = os.path.join(RESULT_ROOT, "hybrid", "combined_failure", "baseline_combined_failure")

LOG_DIR = os.path.join(RESULT_DIR, "hy_baseline_combined_failure_logs")
SUMMARY_FILE = os.path.join(RESULT_DIR, "hy_baseline_combined_failure_summary.csv")
...
WIRED_DELAY_CSV = os.path.join(OMNET_DIR, "results", "wired_delay.csv")
DEFAULT_WIRED_DELAY_SEC = 0.00005236
...
def run_ns3_normal(spacing_m: float):
    cmd = [
        "./ns3",
        "run",
        f"scratch/v2v-wireless-failure --numNodes=5 --distance={spacing_m} --simTime=3"
    ]
                ...
                for i in range(1, VEHICLE_COUNT):
                    ...
                    if node in receivers:
                        if veh in PATH_FAILED_RECEIVERS:
                            ...
                            print(f"[WIRED PATH FAILURE] {veh} wireless received but wired path failed -> no automatic braking")
                            continue
                        ...
                        print(
                            f"[HYBRID] {veh} received and wired path OK, "
                            f"reaction step {reaction_steps[veh]} ({reaction_delay_sec:.8f}s)"
                        )
                    else:
                        reaction_status[veh] = "no_rx"
                        print(f"[DECISION] {veh} -> no_rx, no automatic braking")
