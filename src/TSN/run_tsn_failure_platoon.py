...
print("### TSN-ONLY + PATH FAILURE PLATOON CODE RUNNING - OMNeT++/INET RESULT DRIVEN ###")
...
RESULT_DIR = os.path.join(RESULT_ROOT, "tsn_failure")
LOG_DIR = os.path.join(RESULT_DIR, "tsn_failure_logs")
SUMMARY_FILE = os.path.join(RESULT_DIR, "tsn_failure_summary.csv")
...
OMNET_RUNTIME_RESULT_DIR = os.path.join(
    OMNET_TSN_DIR, "results", "sumo_tsn_failure_runtime"
)
...
# TSN-only에서는 백업 경로가 없으므로 이 차량들은 늦게 반응
...
def decide_tsn_failure_reaction(veh_id: str, omnet_delay_sec: float):
    """
    TSN-only + path failure 모델:
    - 정상 수신 차량은 OMNeT++/INET에서 얻은 TSN critical delay로 반응
    - path failure 차량은 백업 경로가 없으므로 FAILED_REACTION_SEC로 늦게 반응
    """
    if veh_id in PATH_FAILED_RECEIVERS:
        return FAILED_REACTION_SEC, "path_failed_slow_reaction"

    return omnet_delay_sec, "received"
...
def run_single_spacing(spacing_m: float):
    print("\n==============================")
    print(f"TSN-ONLY FAILURE SCENARIO spacing = {spacing_m} m")
    print("==============================")
    ...
    while step < MAX_STEPS:
        ...
        if visible and not event_triggered and all_visible_reference_distance is not None:
            ...
            if (lead_dist - all_visible_reference_distance) >= EVENT_EXTRA_TRAVEL_AFTER_ALL_VISIBLE:
                ...
                print("[OMNeT++] Running INET TSN simulation for TSN-failure base delay...")
                ...
                print(f"[TSN FAILURE] failed receivers = {sorted(PATH_FAILED_RECEIVERS)}")

                for i in range(1, VEHICLE_COUNT):
                    veh = f"car{i}"

                    reaction_delay_sec, status = decide_tsn_failure_reaction(
                        veh, omnet_critical_delay
                    )

                    reaction_steps[veh] = step + max(0, int(reaction_delay_sec / STEP_LENGTH))
                    reaction_status[veh] = status

                    if status == "received":
                        receivers.add(f"node{i}")

                    print(
                        f"[TSN FAILURE OMNET DECISION] {veh} -> {status}, "
            ...
            for i in range(1, VEHICLE_COUNT):
                ...
                if veh in ids and not brake_started[veh]:
                    if step >= reaction_steps.get(veh, 10**9):
                        ...
                        print(f"[TSN FAILURE ACTION] {veh} braking starts at step {step}")
        ...
        if stop_confirmed_step is not None and step >= stop_confirmed_step + POST_STOP_HOLD_STEPS:
            print(f"[END] TSN-only failure scenario {spacing_m}m finished")
            break
...
def main():
    ...
    print("\nDONE: tsn_failure_summary.csv and tsn_failure_logs/*.csv generated")
